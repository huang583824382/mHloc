...
import cv2
import numpy as np
from hloc.my_localize_lib import MainWork, localize_image
import threading
import dash
...
class MyDash(threading.Thread):
    ...
    def run(self):
        print('initing...')
        # global app, fig
        self.app.layout = html.Div([html.Div(id = 'text'), dcc.Interval(id="interval", n_intervals=0)])

        @self.app.callback(Output('text', 'children'), [Input('interval', 'n_intervals')])
        def update_data(n_intervals):
            print('in update_data', n_intervals)
            return 'in update_data'+ str(n_intervals)
...
def main():
    ...
    loop.run_until_complete(SolveQuery())
...
if __name__ == '__main__':
    # t_network = threading.Thread(target=main)
    # t_network.start()
    # t_network.join()
    # main()
    m = MyDash('hello')
